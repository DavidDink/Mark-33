...
MAX_EPSILON = 1
MIN_EPSILON = 0.01
LAMBDA = 0.0075   #how fast the agent becomes less random and chooses the best q-value.
...
def eval_temp_humidity_reward(prev_val, curr_val, desired_val):
    allowed_deviation = 3  # Hard coded - dangerous
    val_in_comfort_zone = within_range(curr_val, desired_val, allowed_deviation)
    if val_in_comfort_zone:
        return 10 # Default reward for when val is in comfort zone
...
sim.setFileSavePath('/Users/ML_Work/Desktop/CSV DATA EVERDAY/')
...
try:
    ...
    for i in range(num_episode):
        ...
        for j in range(num_ep):
            ...
            print("Reward Received:", str(j), reward)
